    ...
    for blob in container_client.list_blobs():
        if (blob.name.endswith('.txt')):
            ...
            if normalised_name not in processed_blobs:
                ...
                if ("raw_unfccc-decisions" in blob.name):

                    raw_text = get_blob_raw_text(blob_connection_string, blob_container_name, blob, skip_to_first_decision=True)

                    if(len(raw_text) > 0):
                        cursor.execute("SELECT COALESCE(MAX(id)+1,1) AS id FROM embed.proceedings_documents")
                        document_id = cursor.fetchone()[0]

                        cursor.execute(
                            "INSERT INTO embed.proceedings_documents (id, title, url) VALUES (%s, %s, %s)",
                            (
                                document_id, 
                        ...
                        for chunk in chunks:
                            ...
                            cursor.execute(
                                "INSERT INTO embed.proceedings_embeddings (vector, content, document_id) VALUES (%s, %s, %s)",
                ...
                # Write individual decisions to a separate vector DB
                elif ("staging_unfccc-decisions" in blob.name):
                    ...
                    if(len(raw_text) > 0):
                        ...
                        cursor.execute("SELECT COALESCE(MAX(id)+1,1) AS id FROM embed.decisions_documents")
                        document_id = cursor.fetchone()[0]

                        cursor.execute(
                            "INSERT INTO embed.decisions_documents (id, title, title_vector, url, created_date) VALUES (%s, %s, %s, %s, %s)",
                            (
                                document_id, 
                                summary[0:200],
                        ...
                        for chunk in chunks:
                            embedding_vector = embedding.embed_query(chunk)
                            cursor.execute(
                                "INSERT INTO embed.decisions_embeddings (vector, content, document_id) VALUES (%s, %s, %s)",
                                (embedding_vector, chunk, document_id)
                            )
